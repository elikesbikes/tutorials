...
### Claude Integration
- **NEVER use the Claude API** — No direct API calls to Anthropic's API in homelab projects
- **ALWAYS use Claude Pro subscription** — Access Claude via:
  - Claude.ai web interface
  - Claude mobile app
  - Claude Code CLI

### Docker Security & Tokens
- **Each app gets its own dedicated token** — Use `claude setup-token` to generate long-lived token stored in `.env`
  - These are app-specific, NOT personal login credentials
  - Tokens don't auto-rewrite
- **No shared `~/.claude` bind-mount** — Remove any bind-mount to `~/.claude` to prevent file corruption across apps
- **Never run containers as root** — All containers must run as non-root user
  - Even if compromised, non-root can't change host file ownership
...
When Claude Code creates a new project:
- **README.md** is generated using `readme-creator` skill (authored as TARS)
- Project documentation follows homelab standards
- README is updated by developer as project evolves
